...
public class AuctionRecordServiceImpl extends BaseFrameworkServiceImpl<AuctionRecordEntity, Long> implements AuctionRecordService {
    ...
    public ResultParam auction(AuctionRecordEntity param, HttpServletRequest request, HttpServletResponse response){
        int ret=0;
        try {
            synchronized (param){
                ret=mapper.insertSelective(param);
            }
            if(ret>0){
    ...
    public void insertESCuriosity(){
        ...
        for (AuctionCommondityEntity commondityEntity:auctionCommondityEntities){
            ...
            if(str!=null&&str!=""){
                ...
                if(new Date().getTime()-auctionRecordEntity.getCreate_time().getTime()>10000){
